...
async function retrievePhoneList() {
        const response = await axios('https://www.ebay.com/sch/i.html?_from=R40&_nkw=iphone+13+pro+unlocked+-max+-cracked+-read+-parts+-board+-box+-Non-Functional+-12+-LCD+-Display+-Shop+-mystery&_sacat=0&_sop=10&rt=nc&LH_BIN=1');
        ...
        phones.each((index, element) => {
            ...
            if(price <= 600 && phoneName != "Shop on eBay Price") { // Creates array of all phones found under price limit
...
setInterval(retrievePhoneList, 15000);
